import { Activity, BarChart2, Command, FileText, Search, Sliders } from 'lucide-react';
...
export function CommandPalette({ isOpen, onClose, onNavigate }: CommandPaletteProps) {
    ...
    const commands = [
        ...
        { id: 'metrics', label: 'Go to Metrics', icon: BarChart2, type: 'navigation' },
        { id: 'controls', label: 'Go to Controls', icon: Sliders, type: 'navigation' },
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center pt-[20vh] px-4">
            ...
            <div className="relative w-full max-w-lg bg-cortex-surface rounded-xl border border-cortex-border shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
                ...
                <div className="max-h-[300px] overflow-y-auto py-2 custom-scrollbar">
                    ...
                    `}
                                    >
                                        <Icon className={`w-4 h-4 ${isSelected ? 'text-white' : 'text-cortex-muted'}`} />
                                        {cmd.label}
                                        {isSelected && (
                                            <span className="ml-auto text-xs text-white/70">Enter</span>
                                        )}
                                    </button>
                                );
                            })}
