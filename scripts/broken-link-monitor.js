...
class BrokenLinkMonitor {
  ...
  saveLinkHistory() {
    try {
      fs.writeFileSync(
        "link-history.json",
        JSON.stringify(this.linkHistory, null, 2),
      );
    } catch (error) {
  ...
  async monitorDocumentationLinks() {
    ...
    this.log(
      `Link monitoring completed: ${this.results.working} working, ${this.results.broken} broken, ${this.results.slow} slow`,
    );
  }
  ...
  updateLinkHistory(links) {
    ...
    links.forEach((link) => {
      ...
      const isBroken = this.brokenLinks.some(
        (broken) => broken.file === link.file && broken.url === link.url,
      );

      const isSlow = this.slowLinks.some(
        (slow) => slow.file === link.file && slow.url === link.url,
      );
  ...
  generateReport() {
    ...
    this.log(
      "Generated broken link reports: broken-link-report.json, broken-link-report.md",
    );
  ...
  generateRecommendations() {
    ...
    if (this.brokenLinks.length > 0) {
      recommendations.push(
        "Fix broken links to improve documentation reliability",
      );
    }

    if (this.slowLinks.length > 0) {
      recommendations.push(
        "Consider replacing slow external links with faster alternatives",
      );
    }

    const externalBroken = this.brokenLinks.filter(
      (link) => link.type === "external",
    );
    if (externalBroken.length > 0) {
      recommendations.push(
        "Review external link dependencies and consider archival alternatives",
      );
    }

    const internalBroken = this.brokenLinks.filter(
      (link) => link.type === "internal",
    );
    if (internalBroken.length > 0) {
      recommendations.push(
        "Update internal links to reflect current file structure",
      );
    }
  ...
  generateMarkdownReport(report) {
    ...
    if (report.brokenLinks.length > 0) {
      ...
      const internalBroken = report.brokenLinks.filter(
        (link) => link.type === "internal",
      );
      const externalBroken = report.brokenLinks.filter(
        (link) => link.type === "external",
      );
    ...
    const healthPercentage = (
      (report.summary.working / report.summary.total) *
      100
    ).toFixed(1);
  ...
  async run() {
    ...
    try {
      ...
      if (report.summary.broken === 0) {
        this.log(
          "Link monitoring completed successfully - no broken links found!",
          "success",
        );
        process.exit(0);
      } else {
        this.log(
          `Link monitoring completed with ${report.summary.broken} broken links`,
          "warning",
        );
        process.exit(1);
      }
      ...
      this.log(
        `Link monitoring failed with unexpected error: ${error.message}`,
        "error",
      );
      process.exit(1);
    }
