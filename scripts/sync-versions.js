...
class VersionSynchronizer {
  ...
  syncVersionInFile(filePath, versionInfo, dryRun = false) {
    ...
    content = content.replace(versionBadgeRegex, (match, prefix, packageName, suffix) => {
      ...
      if (match !== newUrl) {
        this.addChange(filePath, match, newUrl, 'Updated npm version badge URL');
        modified = true;
      }
      return newUrl;
    });
    ...
    const nodeVersionBadgeRegex = /(https:\/\/img\.shields\.io\/badge\/node-)([^-]+)(-[^)\s]*)/g;
    const nodeVersionForBadge = versionInfo.nodeVersion.replace('>=', '%3E%3D');
    
    content = content.replace(nodeVersionBadgeRegex, (match, prefix, version, suffix) => {
      const newUrl = `${prefix}${nodeVersionForBadge}${suffix}`;
      if (match !== newUrl) {
        this.addChange(filePath, match, newUrl, 'Updated Node.js version badge');
        modified = true;
      }
      return newUrl;
    });
    ...
    const nodeRequirementRegex = /(Node\.js\s+)(\d+\.\d+\.\d+|\>=?\d+\.\d+\.\d+)/gi;
    content = content.replace(nodeRequirementRegex, (match, prefix, oldNodeVersion) => {
      if (oldNodeVersion !== versionInfo.nodeVersion) {
        const newMatch = `${prefix}${versionInfo.nodeVersion}`;
        this.addChange(filePath, match, newMatch, 'Updated Node.js version requirement');
        modified = true;
        return newMatch;
      }
      return match;
    ...
    content = content.replace(npmInstallRegex, (match, command, version) => {
      if (version && version !== `@${versionInfo.version}`) {
        ...
        this.addChange(filePath, match, newMatch, 'Updated npm install command');
        modified = true;
        return newMatch;
      }
      return match;
  ...
  syncAllDocumentation(dryRun = false) {
    ...
    this.log(`Synchronizing documentation with version ${versionInfo.version}...`);
  ...
  generateSyncReport() {
    const report = {
      ...
      summary: {
        files_modified: this.changes.length > 0 ? [...new Set(this.changes.map(c => c.file))].length : 0,
    ...
    // Write JSON report
    fs.writeFileSync('version-sync-report.json', JSON.stringify(report, null, 2));

    // Write markdown report
    const markdownReport = this.generateMarkdownSyncReport(report);
    fs.writeFileSync('version-sync-report.md', markdownReport);

    this.log('Generated version sync reports: version-sync-report.json, version-sync-report.md');
  ...
  async run(options = {}) {
    ...
    this.log(`Starting version synchronization${dryRun ? ' (dry run)' : ''}...`);
    ...
    try {
      ...
      if (report.summary.status === 'SUCCESS') {
        if (hasChanges) {
          this.log(`Version synchronization completed with ${report.summary.total_changes} changes`, 'success');
        } else {
          this.log('Version synchronization completed - no changes needed', 'success');
        }
        process.exit(0);
      } else {
        this.log(`Version synchronization failed with ${report.summary.errors} errors`, 'error');
        process.exit(1);
      }
    } catch (error) {
      this.log(`Version synchronization failed with unexpected error: ${error.message}`, 'error');
      process.exit(1);
    }
  }
}
