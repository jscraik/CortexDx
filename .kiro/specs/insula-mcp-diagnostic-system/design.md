...
Insula MCP is built on **FASTMCP v3**, a modular MCP server framework with a plugin architecture that enables:
...
- Enables tools like MCP Inspector to diagnose Insula MCP itself
...
**Design Rationale**: The OAuth authentication system enables Insula MCP to diagnose production MCP servers that require authentication without forcing users to manually extract and provide API keys. The device code flow is particularly important for CLI usage, as it doesn't require browser redirects or complex OAuth callback handling.
...
**ASVS Categories Mapped to Insula MCP**:
...
**High-Priority ATLAS Techniques for Insula MCP** (Req 20.2):
...
**Design Rationale**: Combining OWASP ASVS and MITRE ATLAS provides comprehensive security coverage for both traditional application vulnerabilities and emerging AI/ML threats. This dual-framework approach positions Insula MCP as a security-first diagnostic tool that addresses the unique challenges of AI-powered systems.
...
**Design Rationale**: Supply-chain security is critical for modern applications. By generating SBOMs and continuously monitoring vulnerabilities, Insula MCP helps users maintain secure dependencies. The flict integration ensures legal compliance alongside security.

## Pattern Learning System

### Architecture

The Pattern Learning System enables Insula MCP to learn from successfully applied fixes and automatically resolve recurring problems without re-diagnosing the same issues (Req 16).
...
**Design Rationale**: The pattern learning system addresses the need to avoid re-diagnosing the same issues repeatedly. By storing anonymized problem-solution patterns, Insula MCP can instantly recognize and resolve recurring problems while maintaining user privacy and security.
...
export interface DiagnosticToolSchema {
  name: string; // e.g., "insula_diagnose_protocol"
  description: string;
  inputSchema: {
    type: "object";
    properties: {
...
**Design Rationale**: Exposing plugins as MCP tools enables Insula MCP to be composed with other MCP clients and agents. The orchestration capabilities allow users to build custom diagnostic workflows tailored to their specific needs, while parallel execution ensures fast results.
...
export interface ReportConfig {
  storageRoot: string; // Default: ./reports or $INSULA_REPORT_DIR
