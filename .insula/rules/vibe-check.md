# Vibe Check Oversight Gate — Insula MCP Governance

**Status:** Authoritative  
**Scope:** All human and AI agent executions (planning → action)  
**Inheritance:** Governance Pack (`/.insula/rules/*`), `/.insula/rules/agentic-coding-workflow.md`, `CODESTYLE.md`
...
- `--plan` — ordered steps (≤ 7, numbered). Use `--plan-file` to stream from disk. Link each step to the relevant control ID(s) from `/.insula/rules/llm-threat-controls.md` when elevated risk is identified.
...
The Vibe Check MCP server is now included in the Insula MCP monorepo:
...
4. Map each finding or mitigation to the applicable OWASP LLM Top 10 control ID(s) listed in `/.insula/rules/llm-threat-controls.md`; include the mapping in the PR evidence comment.
...
- **Server unreachable / health check fails:** stop work, mark the task blocked, and escalate per the Constitution. Do **not** bypass the gate without a formally recorded waiver under `/.insula/waivers/`.
...
- Governance reference: `/.insula/rules/agentic-coding-workflow.md` §0.1 and §G5
- CLI helper source: `scripts/oversight/vibe-check-call.mjs`
- Oversight evidence index: `.insula/audit/`
