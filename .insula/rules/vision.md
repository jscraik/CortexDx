...
alwaysApply: true
---
<!-- filename: /.insula/rules/vision.md -->

# Insula MCP Vision Document (v1.1, 2025-11-05)

## 0) North Star (End State)

**Insula MCP** is the stateless, read-only **Model Context Protocol (MCP) Meta-Inspector** that every brAInwav delivery uses to harden MCP servers and clients *before* production. It:
...
4. integrates cleanly with Nx, pnpm, Biome, and Mise so teams can slot Insula into CI/CD without bespoke wiring.

> **Governance is code.** Insula acknowledges the nearest `AGENTS.md`, honors the Constitution + CODESTYLE gates, and treats time freshness and determinism as first-class requirements.
...
- **CLI (`insula-mcp`).** Commands: `diagnose`, `compare`, `doctor`; Commander-based with structured exit codes (`0/1/2`). All probes assume MCP-compatible transports and schema contracts.
...
- Package metadata: `@brainwav/insula-mcp`, Node 20+, sideEffects false, Commander entrypoint.
...
- Shipping multiple MCP servers or managing remote agents; Insula remains a diagnostic client.
