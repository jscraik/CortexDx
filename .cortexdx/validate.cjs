...
function fail(msg) {
	console.error(`[.cortexdx] ${msg}`);
	process.exit(1);
}

function main() {
	const cfgPath = path.resolve(process.cwd(), '.cortexdx/config.yml');
	if (!fs.existsSync(cfgPath)) fail('Missing .cortexdx/config.yml');
	...
	console.log('[.cortexdx] Validation passed');
}
