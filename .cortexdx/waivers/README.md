...
6. **Approval workflow** — File `/.cortexdx/waivers/<waiver_id>.md` including scope, `approved_by` metadata (one or more Maintainers), `expires` (≤ 30 days), mitigation steps, and links to CODESTYLE Appendix B.
...
- The **Agents Governance** workflow now installs Conftest and evaluates `.cortexdx/policy/waivers.rego` with `conftest test -p .cortexdx/policy --all-namespaces reports/waivers.json`. The policy denies:
...
- The command writes YAML to `/.cortexdx/waivers/<id>.yaml` with `created_at` and normalized ISO `expiry` timestamps.
...
- Archive expired waivers under `.cortexdx/waivers/archive/` with outcome notes.
- Escalate to Maintainer if more than two active streaming-handler waivers exist concurrently.

## Ledger & Rotation
- Every approval recorded via the **Apply Waiver** GitHub workflow writes an immutable row to `.cortexdx/audit/waivers.jsonl` capturing the waiver ID, linked PR, approver, timestamps, and workflow metadata.
- Use `node scripts/governance/waiver-ledger.mjs report --ledger .cortexdx/audit/waivers.jsonl --warn-days 7 --format markdown` to review upcoming expirations (also surfaced automatically in the scheduled `waiver-ledger-audit` workflow).
- When a waiver is remediated:
  1. Update the Markdown record under `/.cortexdx/waivers/<id>.md` with `status: closed` and a `closed_at` timestamp, then move it into `/.cortexdx/waivers/archive/`.
  2. Append a closure note to the ledger by re-running `node scripts/governance/waiver-ledger.mjs append --ledger .cortexdx/audit/waivers.jsonl --waiver-id <id> --pr-number <pr> --approver <approver> --note "closed <ISO8601>" --allow-duplicate --actor $(git config user.name)` to preserve an audit trail.
