...
- **REVIEW**
  ...
  - **Merge only if:** **all BLOCKERs** in `/.cortexdx/rules/code-review-checklist.md` are **PASS** and all evidence tokens are present.
...
- **Request:** Draft `/.cortexdx/waivers/<id>.md` citing the charter rule (Agentic Phase Policy REVIEW gate) and the justification, scope, and expiry.
...
- **Action:** Re-run the official scaffolder against the updated templates published on 2025-11-05 — `pnpm task:new --slug <slug> --tier <tier>` — to rebuild `~/.cortexdx/tasks/<slug>/` from the latest manifest.
...
- **Attestation:** Record `TASK_TEMPLATE_REFRESH:2025-11-12 slug=<slug>` in the next `vibe_check` log and cite `/.cortexdx/rules/TASK_FOLDER_STRUCTURE.md` alongside the updated template SHA.
...
- **Automation:** Run `pnpm task:template:check --strict` at every arc boundary; the checker inspects `run-manifest.json → template` and emits `TASK_TEMPLATE_REFRESH:NEEDED slug=<slug>` when the recorded SHA diverges from `/.cortexdx/templates/task-template-version.json`.
