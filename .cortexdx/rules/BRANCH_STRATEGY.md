# CortexDx MCP Branch & Workflow Guide

## 1. Branch Strategy

- **main** — production-ready code only. Anything merged here must pass the full pipeline (`pnpm lint && pnpm test && pnpm build`) and ship green `cortexdx diagnose` artifacts.
...
```bash
git clone git@github.com:.../cortexdx.git
cd cortexdx
...
4. Follow ArcTDD:
   - Write or adjust the Vitest spec in `packages/cortexdx/tests`.
   - Implement code under `packages/cortexdx/src`.
...
5. Local checks:
   ...
   npx cortexdx diagnose http://localhost:8088 --out reports/dev  # optional mock validation
   ```
...
- GitHub Action runs `pnpm build`, `pnpm lint`, `pnpm test`, then `cortexdx diagnose`. Blockers or majors fail the job unless explicitly flagged.
...
Follow this rhythm to maintain a clear separation between production-ready (`main`), staging (`develop`), and feature experimentation, aligned with CortexDx MCP governance and tooling.
