...
```text
cortex-os-clean/
├── .cortexdx/                          # Governance hub (single source of truth)
...
All governance lives in `.cortexdx/`:

1. **Rules** (`.cortexdx/rules/`): Human-readable policies.
2. **Policies** (`.cortexdx/policy/`): Machine-readable JSON.
3. **Schemas** (`.cortexdx/schemas/`): Validation schemas.
4. **Gates** (`.cortexdx/gates/`): Enforcement scripts.
...
**Note**: This document is maintained in `.cortexdx/docs/project-structure.md` and is the single source of truth for project organization. All agents should reference this document when making architectural decisions.
