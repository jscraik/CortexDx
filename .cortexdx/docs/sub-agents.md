...
- **Project subagent**: `.cortexdx/agents/` (highest priority)
- **User subagent**: `~/.cortexdx/agents/` (lower priority)
...
| :---------- | :------------------ | :---------------------------- | :------- |
| **Project** | `.cortexdx/agents/`   | Available in current project  | Highest  |
| **User**    | `~/.cortexdx/agents/` | Available across all projects | Lower    |
...
```bash
mkdir -p .cortexdx/agents
$EDITOR .cortexdx/agents/test-runner.md
```

All subagent files are validated against JSON Schemas in `.cortexdx/schemas/agents/*.schema.json`.
...
- **Version control** project subagents in `.cortexdx/agents`
