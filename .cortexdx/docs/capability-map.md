...
- Security/Gov: Policies in `.cortexdx/` with CI gates, SBOM+signing.
...
- FS writes: Sandbox to `/var/cortex/tmp` and `./.cortexdx/tmp`.
- Structure: `.cortexdx/policy/structure.json` + `tools/structure-guard` pre-commit + CI.
