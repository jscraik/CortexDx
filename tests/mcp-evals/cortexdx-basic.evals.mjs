...
const defaultEndpoint =
  process.env.CORTEXDX_EVAL_ENDPOINT ?? "http://127.0.0.1:5001/mcp";
const baseInstructions =
  `You are testing the CortexDx MCP server at ${defaultEndpoint}. ` +
  "Use CortexDx tools directly (diagnose_mcp_server, deepcontext, orchestration) and summarize findings.";

const serverPath =
  process.env.CORTEXDX_EVAL_STDIO ??
  process.argv[3] ??
...
const diagnoseEval = {
  ...
  description:
    "Ensures diagnose_mcp_server reports blocker/major/minor counts with evidence.",
  run: async (model) => {
    const prompt =
      `${baseInstructions} Call diagnose_mcp_server on ${defaultEndpoint} and summarize severities ` +
...
const deepcontextEval = {
  ...
  description:
    "Validates DeepContext CLI tooling returns index stats and determinism info.",
  run: async (model) => {
    const prompt =
      `${baseInstructions} Verify deepcontext tools (status + search) respond without errors ` +
...
const orchestrationEval = {
  ...
  description:
    "Checks LangGraph orchestration tooling executes a workflow and reports spans.",
  run: async (model) => {
    const prompt =
      `${baseInstructions} Run the orchestrate workflow agent.langgraph.baseline and summarize ` +
