...
export function createOrchestrationPlugin(
  config: OrchestrationPluginConfig = {},
): ServerPlugin {
  ...
  return {
    ...
    async onToolCall(ctx: RequestContext, toolName: string, args: unknown) {
      if (toolName === "set_model") {
        ...
        ctx.fastMCP?.log?.info(
          `Processing chat completion with model ${currentModel}`,
        );
      }
    },
  };
...
export const orchestrationTools = {
  set_model: {
    ...
    parameters: z.object({
      model: z
        .string()
        .describe("Model ID to switch to (e.g., gpt-4o, claude-3-5-sonnet)"),
    }),
        ...
        choices: [
          {
            ...
            message: {
              role: "assistant",
              content:
                "This is a simulated response from the orchestration plugin. In a real implementation, this would call the LLM provider.",
            },
            finish_reason: "stop",
          },
        ],
