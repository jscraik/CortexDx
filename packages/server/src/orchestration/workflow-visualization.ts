...
export class WorkflowVisualizationEngine {
  ...
  generateMermaidDiagram(
    ...
    for (const node of definition.nodes) {
      ...
      lines.push(
        `    ${node.id}${nodeShape[0]}${nodeLabel}${nodeShape[1]}${nodeStyle}`,
      );
    }
    ...
    if (opts.highlightCurrentNode && state?.currentNode) {
      lines.push(
        `    style ${state.currentNode} fill:#ffeb3b,stroke:#f57c00,stroke-width:4px`,
      );
    }
    ...
    if (opts.showVisitedNodes && state?.visitedNodes) {
      for (const visitedNode of state.visitedNodes) {
        if (visitedNode !== state.currentNode) {
          lines.push(
            `    style ${visitedNode} fill:#c8e6c9,stroke:#4caf50,stroke-width:2px`,
          );
        }
      }
    }
   ...
   * Requirements: 18.5
   */
  generateExecutionHighlight(diagram: string, state: WorkflowState): string {
   ...
   * Requirements: 18.5
   */
  generateTransitionHistory(transitions: StateTransition[]): string {
    const lines: string[] = [];
    ...
    for (let i = 0; i < transitions.length; i++) {
      ...
      if (transition) {
        ...
        lines.push(
          `    ${fromNode}_${i} --> |${transition.transitionType}${duration}| ${toNode}_${i}`,
        );
      }
    }
  ...
  generateMetricsSummary(metrics: Map<string, NodeMetrics>): string {
    ...
    lines.push(
      "| Node | Executions | Avg Time | Min Time | Max Time | Errors |",
    );
    lines.push(
      "|------|------------|----------|----------|----------|--------|",
    );
  ...
  private getNodeStyleString(
    ...
    if (customStyle && (customStyle.fill || customStyle.stroke)) {
      ...
      if (customStyle.strokeWidth)
        parts.push(`stroke-width:${customStyle.strokeWidth}px`);
      return `:::${parts.join(",")}`;
    }
        ...
        return JSON.stringify(
          {
            ...
            metrics: Array.from(visualization.metrics.entries()).map(
              ([id, metricsData]) => ({ ...metricsData, nodeId: id }),
            ),
