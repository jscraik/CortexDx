...
export class StateManager {
  ...
  async saveCheckpoint(checkpoint: StateCheckpoint): Promise<void> {
    try {
      ...
      const message = error instanceof Error ? error.message : String(error);
      throw new Error(
        `Failed to save checkpoint ${checkpoint.checkpointId}: ${message}`,
      );
    }
  }
  ...
  async recoverState(
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : String(error);
      throw new Error(
        `Failed to recover state for workflow ${options.workflowId}: ${message}`,
      );
    }
  }
  ...
  async createSession(
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : String(error);
      throw new Error(
        `Failed to create session for workflow ${workflowId}: ${message}`,
      );
    }
  }
  ...
  async listSessions(
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : String(error);
      throw new Error(
        `Failed to list sessions for workflow ${workflowId}: ${message}`,
      );
    }
  }
  ...
  async updateSessionStatus(
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : String(error);
      throw new Error(
        `Failed to update session ${sessionId} status: ${message}`,
      );
    }
  }
  ...
  async recordTransition(
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : String(error);
      throw new Error(
        `Failed to record transition for checkpoint ${checkpointId}: ${message}`,
      );
    }
  }
  ...
  async getTransitionHistory(
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : String(error);
      throw new Error(
        `Failed to get transition history for workflow ${workflowId}: ${message}`,
      );
    }
  }
  ...
  enableAutoSave(
    ...
    const timer = setInterval(async () => {
      try {
        ...
        const message = error instanceof Error ? error.message : String(error);
        console.error(
          `[StateManager] Auto-save failed for ${workflowId}/${threadId}: ${message}`,
        );
      }
    }, intervalMs);
  ...
  private async updateSession(
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : String(error);
      throw new Error(
        `Failed to update session for workflow ${workflowId}: ${message}`,
      );
    }
  }
  ...
  private async cleanupOldCheckpoints(
    ...
    try {
      ...
      console.error(
        `[StateManager] Failed to cleanup old checkpoints for ${workflowId}/${threadId}: ${message}`,
      );
    }
  }
}
...
export function getStateManager(config?: StatePersistenceConfig): StateManager {
  ...
  if (!instance) {
    if (!config) {
      throw new Error(
        `StateManager not initialized for key ${key}. Provide config on first call.`,
      );
    }
