...
export class AgentOrchestrator {
  ...
  createWorkflow(definition: WorkflowDefinition): CompiledWorkflow {
    ...
    const graphBuilder = new StateGraph<WorkflowState>({
      channels: {
        ...
        context: {
          value: (x: DiagnosticContext, y?: DiagnosticContext) => y ?? x,
        },
        config: { value: (x: WorkflowConfig, y?: WorkflowConfig) => y ?? x },
        startTime: { value: (x: number, y?: number) => y ?? x },
        nodeTimings: {
          value: (x: Record<string, number>, y?: Record<string, number>) => ({
            ...x,
            ...y,
          }),
        },
  ...
  async executeWorkflow(
    ...
    if (!initialState.context) {
      throw new Error(
        "Workflow execution requires an initial diagnostic context",
      );
    }
    ...
    if (options?.streamEvents) {
      ...
      finalState = fromRecord<WorkflowState>(result, [
        "endpoint",
        "findings",
        "errors",
      ]);
