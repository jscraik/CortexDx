...
export class AssertionEmitter {
  ...
  build(): Assertion {
    ...
    // TypeScript can't infer that these fields are now guaranteed to be defined
    // after the validation above, so we need to assert their types
    const assertion: Assertion = {
      id: this.assertion.id as string,
      title: this.assertion.title as string,
      status: this.assertion.status as AssertionStatus,
