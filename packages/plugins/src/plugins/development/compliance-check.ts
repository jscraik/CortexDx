  ...
  try {
    ...
    if (!initResponse.success) {
      findings.push({
        ...
        description:
          "Server does not properly respond to MCP initialize requests.",
        evidence: [{ type: "url", ref: `${endpoint}/mcp` }],
        recommendation:
          "Implement proper MCP initialize method according to protocol specification.",
        remediation: {
          steps: [
      ...
      if (initResponse.protocolVersion !== "2024-11-05") {
        findings.push({
          ...
          evidence: [{ type: "url", ref: `${endpoint}/mcp` }],
          recommendation:
            "Update to the latest MCP protocol version for best compatibility.",
        });
      }
      ...
      if (
        ...
        findings.push({
          ...
          description:
            "Server capabilities are not properly structured according to MCP specification.",
          evidence: [{ type: "url", ref: `${endpoint}/mcp` }],
          recommendation:
            "Ensure capabilities object includes tools, resources, and prompts properties.",
        });
      }
    }
    ...
    if (!toolsResponse.success) {
      findings.push({
        ...
        evidence: [{ type: "url", ref: `${endpoint}/mcp` }],
        recommendation:
          "Implement tools/list method to expose available tools.",
      });
    }
  } catch (error) {
    findings.push({
      ...
      recommendation:
        "Ensure the MCP server is running and accessible at the specified endpoint.",
    });
  }
...
function checkProjectCompliance(projectContext: ProjectContext): Finding[] {
  ...
  if (type === "mcp-server" && language === "typescript") {
    const hasMcpSdk = dependencies.some(
      (dep: string) =>
        dep.includes("@modelcontextprotocol/sdk") || dep.includes("mcp-sdk"),
    );
    ...
    if (!hasMcpSdk) {
      findings.push({
        ...
        description:
          "Project does not include the official MCP SDK dependency.",
        evidence: [{ type: "file", ref: "package.json" }],
        recommendation:
          "Consider using the official MCP SDK for better protocol compliance and easier development.",
      });
    }
  }
  ...
  const requiredConfigs =
    type === "mcp-server" ? ["package.json"] : ["package.json"];
  const missingConfigs = requiredConfigs.filter(
    (config) => !configFiles.includes(config),
  );
  ...
  if (missingConfigs.length > 0) {
    findings.push({
      ...
      recommendation:
        "Ensure all required configuration files are present for proper MCP implementation.",
    });
  }
...
export const ComplianceCheckPlugin: DevelopmentPlugin = {
  ...
  async run(ctx: DevelopmentContext): Promise<Finding[]> {
    ...
    try {
      ...
      const hasComplianceQuestions = recentMessages.some(
        (msg) =>
      ...
      if (hasComplianceQuestions) {
        findings.push({
          ...
          description:
            "Recent conversation indicates questions about MCP protocol compliance.",
          evidence: [{ type: "log", ref: "conversation-history" }],
          recommendation:
            "I can help ensure your MCP implementation follows the protocol specifications. Would you like me to run a detailed compliance check?",
        });
      }
    } catch (error) {
      findings.push({
        ...
        recommendation:
          "Ensure the MCP server is accessible and implements the required protocol endpoints.",
      });
    }
