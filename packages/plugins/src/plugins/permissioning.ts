...
const RULES: Array<{ re: RegExp; tag: string; severity: "major" | "minor" }> = [
  {
    re: /\b(exec|shell|bash|sh|powershell)\b/i,
    tag: "exec",
    severity: "major",
  },
  {
    re: /\b(fs|filesystem|write|delete|rm|unlink|chmod|chown)\b/i,
    tag: "filesystem",
    severity: "major",
  },
  {
    re: /\b(sql|sqlite|postgres|mysql|query)\b/i,
    tag: "sql",
    severity: "major",
  },
  {
    re: /\b(curl|http|get|post|fetch|open[-_ ]?url)\b/i,
    tag: "net",
    severity: "minor",
  },
];
  ...
  async run(ctx) {
    ...
    for (const tool of tools) {
      for (const hit of classify(tool)) {
        if (hit.tag === "sql" && tool.name && SQL_ALLOWLIST.has(tool.name))
          continue;
        flagged += 1;
        findings.push({
    ...
    if (flagged === 0) {
      findings.push({
        ...
        description:
          "Heuristics found no risky verbs in tool names/descriptions.",
        evidence: [{ type: "url", ref: ctx.endpoint }],
      });
    }
