...
export const createInstrumentedDevelopmentTools = () => {
    ...
    const diagnoseMcpServer = instrumentTool('diagnose_mcp_server', async (args: unknown, ctx: unknown) => {
        ...
        recordMetric('diagnostic.started', 1, true, { endpoint, suites: suites.length, full });
    ...
    const startConversation = instrumentTool('start_conversation', async (args: unknown) => {
    ...
    const validateLicense = instrumentTool('validate_license', async (args: unknown) => {
...
export const createInstrumentedJsonRpcHandlers = () => {
    ...
    const handleInitialize = instrumentJsonRpc('initialize', async (responseId: unknown) => {
    ...
    const handleToolsList = instrumentJsonRpc('tools/list', async (responseId: unknown) => {
    ...
    const handleToolsCall = instrumentJsonRpc('tools/call', async (params: unknown, responseId: unknown) => {
