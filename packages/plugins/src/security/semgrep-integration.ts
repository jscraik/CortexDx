...
export class SemgrepIntegration {
   ...
   */
  async scanCode(path: string, rules: SemgrepRule[]): Promise<SemgrepResults> {
    if (rules.length === 0) {
  ...
  async detectWeakAuthentication(
    ...
    if (!ctx.headers || Object.keys(ctx.headers).length === 0) {
      findings.push({
        ...
        column: 1,
        message:
          "No authentication headers detected. Implement OAuth 2.0 or API key authentication.",
        severity: "WARNING",
        code: "headers: {}",
      });
    }
    ...
    return [
      ...
      },
      {
        ...
        metadata: {
          ...
          atlas: "AML.T0051",
          mitigation:
            "Sanitize and validate all user inputs before LLM processing",
        },
      },
      {
      ...
      },
      {
        ...
        message:
          "Weak cryptographic algorithm MD5 detected. Use SHA-256 or better.",
        severity: "WARNING",
        languages: ["typescript", "javascript"],
        metadata: {
          category: "security",
      ...
      },
      {
        ...
        message:
          "Direct JSON.parse on untrusted input. Use safeParseJson() + schema validation.",
        severity: "WARNING",
        languages: ["typescript", "javascript"],
        metadata: {
          category: "security",
  ...
  private buildSemgrepConfig(rules: SemgrepRule[]): string {
    const ruleBlocks = rules
      .map((rule) => {
        const languages = rule.languages.map((lang) => `'${lang}'`).join(", ");
...
${patternBody}
  ...
  private parseSemgrepJson(payload: string): {
    findings: SemgrepFinding[];
    errors: string[];
  } {
    try {
      ...
      const findings =
        data.results?.map((result) => {
          ...
          return {
            ...
            severity:
              (result.extra?.severity?.toUpperCase() as SemgrepFinding["severity"]) ||
              "WARNING",
