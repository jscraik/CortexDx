...
export class ASVSComplianceEngine {
  ...
  private async validateAuthentication(
    ...
    const hasAuth = Boolean(ctx.headers && Object.keys(ctx.headers).length > 0);
  ...
  private findASVSMapping(finding: Finding): string[] {
    ...
    if (
      finding.tags?.includes("validation") ||
      finding.tags?.includes("injection")
    ) {
      mappings.push("V5");
    }
    if (
      finding.tags?.includes("cryptography") ||
      finding.tags?.includes("encryption")
    ) {
      mappings.push("V7");
    }
    if (
      finding.tags?.includes("communication") ||
      finding.tags?.includes("tls")
    ) {
      mappings.push("V9");
    }
