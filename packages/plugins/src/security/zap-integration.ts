...
export class ZAPIntegration {
  ...
  async scanMCPTransport(ctx: DiagnosticContext): Promise<ZAPFinding[]> {
    ...
    if (
      ctx.endpoint.startsWith("http://") ||
      ctx.endpoint.startsWith("https://")
    ) {
  ...
  private async invokeZap(
    ...
    return {
      ...
      informational: findings.filter((f) => f.riskLevel === "Informational")
        .length,
 ...
 */
export function normalizeZAPResults(
  results: ZAPResults,
): NormalizedZAPFinding[] {
