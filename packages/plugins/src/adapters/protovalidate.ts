...
export class ProtovalidateAdapter {
  ...
  async validate(
    ...
    const validationPromises = constraints.map(async (rule) => {
      try {
        const result = await this.evaluateCEL(
          rule.expression,
          message,
          rule.field,
        );
  ...
  private generateCacheKey(message: unknown, constraints: CELRule[]): string {
    ...
    const constraintsHash = constraints
      .map((c) => `${c.field}:${c.expression}`)
      .join("|");
  ...
  async validateField(
    ...
    if (typeof value === "string" && this.isStringConstraints(constraint)) {
      const stringViolations = this.validateString(
        value,
        fieldPath,
        constraint,
      );
      ...
      typeof value === "number" &&
      this.isNumberConstraints(constraint)
    ) {
      const numberViolations = this.validateNumber(
        value,
        fieldPath,
        constraint,
      );
      ...
      const arrayViolations = this.validateRepeated(
        value,
        fieldPath,
        constraint,
      );
   ...
   * Type guards
   */
  private isStringConstraints(
    constraint: unknown,
  ): constraint is StringConstraints {
    return (
      typeof constraint === "object" &&
      constraint !== null &&
      ("minLength" in constraint ||
        "maxLength" in constraint ||
        "pattern" in constraint)
    );
  }

  private isNumberConstraints(
    constraint: unknown,
  ): constraint is NumberConstraints {
    return (
      typeof constraint === "object" &&
      constraint !== null &&
  ...
  private isRepeatedConstraints(
    constraint: unknown,
  ): constraint is RepeatedConstraints {
    return (
      typeof constraint === "object" &&
      constraint !== null &&
      ("minItems" in constraint ||
        "maxItems" in constraint ||
        "unique" in constraint)
    );
  }
  ...
  private evaluateSize(
    ...
    if (target === "this") {
      ...
      if (typeof targetValue === "string" || Array.isArray(targetValue)) {
        actualSize = (targetValue as string | unknown[]).length;
  ...
  private evaluateComparison(
    ...
    for (const op of operators) {
      if (expr.includes(op)) {
        ...
        const left =
          leftPart === "this" ? value : this.parseValue(leftPart, context);
        const right = this.parseValue(rightPart, context);
