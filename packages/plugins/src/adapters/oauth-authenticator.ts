...
export class OAuthAuthenticator {
  ...
  async initiateOAuth2Flow(
    ...
    try {
      ...
      throw new Error(
        `Failed to initiate OAuth2 flow for ${serverEndpoint}: ${message}`,
      );
    }
  }
  ...
  async deviceCodeFlow(
    ...
    try {
      ...
      if (elapsed > 2000) {
        throw new Error(`Device code flow exceeded 2s threshold: ${elapsed}ms`);
      }
  ...
  detectAuthRequirement(
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : String(error);
      console.warn(
        `[oauth-authenticator] Failed to detect auth requirement: ${message}`,
      );
  ...
  getSession(sessionId: string): OAuth2Session | undefined {
    try {
      ...
      const message = error instanceof Error ? error.message : String(error);
      console.warn(
        `[oauth-authenticator] Failed to get session ${sessionId}: ${message}`,
      );
      return undefined;
    }
  }
  ...
  updateSessionStatus(
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : String(error);
      console.warn(
        `[oauth-authenticator] Failed to update session ${sessionId}: ${message}`,
      );
    }
  }
  ...
  clearSession(sessionId: string): void {
    try {
      ...
      const message = error instanceof Error ? error.message : String(error);
      console.warn(
        `[oauth-authenticator] Failed to clear session ${sessionId}: ${message}`,
      );
    }
  }
