...
function categorizeError(fieldPath: string): ErrorCategory {
  ...
  if (
    fieldPath.includes("jsonrpc") ||
    fieldPath.includes("method") ||
...
function generateSuggestion(
  ...
  > = {
    ...
    handshake: () =>
      "Ensure the initialize request/response follows the MCP handshake protocol",

    authentication: () =>
      "Check authentication configuration and ensure proper credentials are provided",

    general: (v) =>
      `Fix the ${v.fieldPath} field to satisfy the constraint: ${v.expectedConstraint}`,
  };
...
async function initializeDocUrlCache(): Promise<void> {
  try {
    ...
    console.error(
      "[initializeDocUrlCache] Failed to initialize documentation URL cache:",
      err,
    );
  }
}
...
function formatErrorsAsMarkdown(errors: DetailedError[]): string {
  ...
  for (const error of errors) {
    ...
    lines.push("");
    lines.push(
      `**Documentation**: [${error.category}](${error.documentation})`,
    );
    lines.push("");
    lines.push("---");
    lines.push("");
...
function formatErrorsAsHTML(errors: DetailedError[]): string {
  ...
  for (const error of errors) {
    ...
    if (error.violation.value !== undefined) {
      ...
      lines.push(
        `<pre>${JSON.stringify(error.violation.value, null, 2)}</pre>`,
      );
      lines.push("</div>");
    }

    lines.push(
      `<p><strong>Expected:</strong> ${error.violation.expectedConstraint}</p>`,
    );
    lines.push(`<p><strong>Suggestion:</strong> ${error.suggestion}</p>`);
    lines.push(
      `<p><strong>Documentation:</strong> <a href="${error.documentation}" target="_blank">${error.category}</a></p>`,
    );
    lines.push("</div>");
  }
