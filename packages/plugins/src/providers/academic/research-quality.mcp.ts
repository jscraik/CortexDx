...
export class ResearchQualityProvider {
  ...
  constructor(private ctx: DiagnosticContext) {
    ...
    this.remoteClient = initializeVibeCheckClient(ctx);
  }
        ...
        inputSchema: {
          ...
          properties: {
            venue_name: {
              type: "string",
              description:
                "Name of the journal, conference, or publication venue",
            },
            issn: {
              type: "string",
        ...
        inputSchema: {
          ...
          properties: {
            ...
            research_type: {
              type: "string",
              description:
                "Type of research (experimental, observational, theoretical, etc.)",
              enum: [
                "experimental",
                "observational",
                "theoretical",
  ...
  async assessQuality(
    ...
    if (params.text) {
      const reproducibilityScore = await this.assessReproducibility(
        params.text,
      );
    ...
    assessment.metrics.overall_score = this.calculateOverallScore(
      assessment.metrics,
    );
    assessment.metrics.confidence_level =
      this.calculateConfidenceLevel(assessment);

    // Generate recommendations based on flags
    assessment.recommendations.push(
      ...this.generateRecommendations(assessment.flags),
    );
  ...
  private async assessMethodology(
    ...
    if (
      /experiment|trial|intervention/i.test(text) &&
      !/control|placebo|comparison/i.test(text)
    ) {
      flags.push({
        type: "methodology",
        severity: "high",
        ...
        recommendation:
          "Include appropriate control groups in experimental design",
      });
      score -= 25;
    }
  ...
  private async assessVenue(
    ...
    const isPredatory = this.predatoryPatterns.some((pattern) =>
      pattern.test(venueName),
    );
    ...
    if (isPredatory) {
      ...
      flags.push({
        ...
        recommendation:
          "Verify venue legitimacy through reputable indexing services",
      });
    }
   ...
   * Assess statistical rigor
   */
  private async assessStatisticalRigor(
    text: string,
  ): Promise<{ score: number; flags: QualityFlag[] }> {
    const flags: QualityFlag[] = [];
    let score = 75;
    ...
    if (/p\s*[<>=]\s*0\.05|p\s*[<>=]\s*\.05/i.test(text)) {
      ...
      const suspiciousPValues = pValues.filter((p) =>
        /0\.04[5-9]|0\.05/i.test(p),
      );
      ...
      if (suspiciousPValues.length > 2) {
        flags.push({
          ...
          recommendation:
            "Consider multiple comparison corrections and report effect sizes",
        });
        score -= 20;
      }
    }
   ...
   * Assess reproducibility
   */
  private async assessReproducibility(
    text: string,
  ): Promise<{ score: number; flags: QualityFlag[] }> {
    const flags: QualityFlag[] = [];
    let score = 70;
    ...
    if (
      !/data.*available|supplementary.*data|repository|github|zenodo/i.test(
        text,
      )
    ) {
      flags.push({
        ...
        recommendation:
          "Provide data availability statement and share data when possible",
      });
      score -= 20;
    }
    ...
    if (
      !/code.*available|software|github|algorithm.*implementation/i.test(text)
    ) {
      flags.push({
        type: "reproducibility",
        severity: "medium",
   ...
   * Assess ethical compliance
   */
  private async assessEthicalCompliance(
    text: string,
  ): Promise<{ score: number; flags: QualityFlag[] }> {
    const flags: QualityFlag[] = [];
    let score = 85;
    ...
    if (
      /participant|subject|human|patient/i.test(text) &&
      !/ethics.*approval|irb|institutional.*review/i.test(text)
    ) {
      flags.push({
        type: "ethical",
        severity: "high",
        ...
        recommendation:
          "Include ethics approval information for human subjects research",
      });
      score -= 30;
    }
    ...
    if (
      /participant|subject|human/i.test(text) &&
      !/consent|informed/i.test(text)
    ) {
      flags.push({
        type: "ethical",
        severity: "medium",
  ...
  private calculateConfidenceLevel(assessment: QualityAssessment): number {
    ...
    const highFlags = assessment.flags.filter(
      (f) => f.severity === "high",
    ).length;
  ...
  async generateRefactoringSuggestions(
    ...
    if (focusSet.has("maintainability")) {
      ...
      if (functionCount > 20) {
        recommendations.push(
          "High function count - consider organizing into classes or modules",
        );
      }
    }
    ...
    if (focusSet.has("performance")) {
      if (/for\s*\([^)]*\)\s*{[^}]*for\s*\(/g.test(code)) {
        recommendations.push(
          "Nested loops detected - consider optimization or caching",
        );
      }
    }
    ...
    if (focusSet.has("security")) {
      if (/eval\s*\(|new\s+Function\s*\(/g.test(code)) {
        recommendations.push(
          "Avoid eval() and Function constructor for security",
        );
      }
    }
    ...
    if (focusSet.has("readability")) {
      ...
      if (avgLineLength > 100) {
        recommendations.push(
          "Long lines detected - consider breaking for readability",
        );
      }
    }
    ...
    return {
      ...
      recommendations,
      estimatedEffort:
        antiPatterns.length > 10
          ? "high"
          : antiPatterns.length > 5
            ? "medium"
            : "low",
      priority: antiPatterns.some((ap) => ap.severity === "critical")
        ? "high"
        : antiPatterns.some((ap) => ap.severity === "high")
          ? "medium"
          : "low",
    };
  }
  ...
  async analyzeCodeHealth(
    ...
    const maintainability = Math.min(
      100,
      (commentLines / lines.length) * 200 + 50,
    );
    ...
    const reliability = Math.max(
      ...
      100 -
        antiPatterns.filter(
          (ap) => ap.severity === "high" || ap.severity === "critical",
        ).length *
          15,
    );

    const security = /eval\(|innerHTML|dangerouslySetInnerHTML/.test(code)
      ? 50
      : 90;
  ...
  private async callRemoteTool<T = unknown>(
    name: string,
    ...
    try {
      ...
      this.ctx.logger?.(
        `[VibeCheck] Remote tool ${name} failed: ${String(error)}`,
      );
      return null;
    }
  }
...
function initializeVibeCheckClient(
  ctx: DiagnosticContext,
): HttpMcpClient | undefined {
  if (process.env.CORTEXDX_DISABLE_VIBECHECK_HTTP === "1") {
    return undefined;
  }
  const headerBase = ctx.headers?.["vibe-check-url"] as string | undefined;
  const baseUrl = (headerBase ?? process.env.VIBE_CHECK_HTTP_URL)?.trim();
  if (!baseUrl) {
    return undefined;
  }
  const profile =
    (ctx.headers?.["x-vibe-profile"] as string | undefined) ??
    process.env.VIBE_CHECK_PROFILE;
  const strictHeader =
    (ctx.headers?.["x-vibe-strict"] as string | undefined) ??
    process.env.VIBE_CHECK_STRICT;
  const apiKey =
    (ctx.headers?.["x-vibe-api-key"] as string | undefined) ??
    process.env.VIBE_CHECK_API_KEY;
  try {
    return new HttpMcpClient({
      baseUrl,
      headers: {
        "X-Vibe-Client": "cortexdx",
        ...(profile ? { "X-Vibe-Profile": profile } : {}),
        ...(strictHeader ? { "X-Vibe-Strict": strictHeader } : {}),
        ...(apiKey ? { "X-Vibe-Api-Key": apiKey } : {}),
      },
    });
  } catch (error) {
    console.warn("[VibeCheck] Failed to initialize HTTP MCP client:", error);
    return undefined;
  }
}
