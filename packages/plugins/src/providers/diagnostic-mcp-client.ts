...
} from "./academic/http-mcp-client.js";
...
export async function createDiagnosticMcpClient(
  ...
  // Method 2: Create new diagnostic session using Auth0
    ...
    const sessionConfig: Omit<DiagnosticSessionConfig, "metadata"> =
    ...
    throw new Error(
      "Either diagnosticSessionKey or auth0 config must be provided",
    );
  }
...
async function createDiagnosticSession(
  ...
  const response = await fetch(
    `${targetServerUrl}/api/v1/diagnostic-session/create`,
    {
      method: "POST",
      headers: {
  ...
  if (!result.success || !result.session) {
    throw new Error(
      `Failed to create diagnostic session: ${result.error || "Unknown error"}`,
    );
  }
