...
describe("KnowledgeRAG", () => {
    ...
    it("indexes spec content by chunking and storing embeddings", async () => {
        ...
        const storedDocs = (mockStorage.addDocuments as any).mock.calls[0][0] as VectorDocument[];
        if (!storedDocs || !storedDocs[0] || !storedDocs[1]) throw new Error("Docs not stored");
    ...
    it("searches for content using embeddings", async () => {
        ...
        const mockQueryEmbedding = { values: [0.5], dimensions: 1, model: "test", timestamp: 0 };
        ...
        expect(mockEmbedding.embed).toHaveBeenCalledWith({ text: query, normalize: true });
        expect(mockStorage.search).toHaveBeenCalledWith(mockQueryEmbedding, { topK: 5, filterType: "reference" });
