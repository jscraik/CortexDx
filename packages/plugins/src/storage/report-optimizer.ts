...
export function optimizeResponse(
  ...
  return {
    ...
    findingsSummary,
    criticalFindings:
      criticalFindings.length > 0 ? criticalFindings : undefined,
    metadata: {
...
export function generateSummary(
  ...
  parts.push(
    `Found ${summary.total} issue${summary.total === 1 ? "" : "s"} in ${report.diagnosticType} diagnostic:`,
  );
...
export function filterReports(
  ...
  if (options.diagnosticType) {
    filtered = filtered.filter(
      (r) => r.diagnosticType === options.diagnosticType,
    );
  }
...
export function estimateTokenSavings(report: DiagnosticReport): {
  ...
  const savingsPercent =
    fullReportTokens === 0 ? 0 : (savings / fullReportTokens) * 100;
