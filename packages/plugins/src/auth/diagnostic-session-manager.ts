...
export class DiagnosticSessionManager {
    ...
    constructor(dbPath?: string) {
        this.dbPath = dbPath || join(process.cwd(), '.cortexdx', 'diagnostic-sessions.db');
     ...
     */
    listSessions(requestedBy: string, status?: "active" | "revoked" | "expired"): DiagnosticSession[] {
        let query = `
            SELECT * FROM diagnostic_sessions
            WHERE requested_by = ?
        `;
     ...
     * Track session usage
     */
    trackUsage(sessionId: string, endpoint: string, method: string, ipAddress?: string, userAgent?: string): void {
        const stmt = this.db.prepare(`
