    ...
    it("should validate generated code against requirements", () => {
        ...
        const meetsRequirements = requirements.every(req => {
            ...
            return implementation.features.some(f => {
                ...
                if (normalized.includes(f.toLowerCase()) || normalized.includes(featureNormalized)) {
                    return true;
                }
                ...
                return featureWords.every(word => {
                    ...
                    return normalized.includes(word) ||
                        normalized.split(" ").some(reqWord =>
                            reqWord.startsWith(word.slice(0, -3)) || word.startsWith(reqWord.slice(0, -3))
                        );
                });
            });
        });
