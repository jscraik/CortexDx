...
describe('CLI Self-Healing Commands', () => {
  ...
  describe('self-diagnose command', () => {
    it('should run self-diagnose with correct options', async () => {
      const { runSelfDiagnose } = await import('../src/commands/self-healing.js');
      clearLogs();
  ...
  describe('heal command', () => {
    it('should run heal endpoint with correct parameters', async () => {
      const { runHealEndpoint } = await import('../src/commands/self-healing.js');
      clearLogs();
  ...
  describe('monitor command', () => {
    ...
    it('should load configuration from file', async () => {
      ...
      // Note: start operation never resolves, so we'll test config loading
      // by verifying it reads the config file
      const startPromise = runMonitoring({
        start: true,
        config: 'monitor-config.json',
      });

      // Give it a moment to read the config
      await new Promise(resolve => setTimeout(resolve, 10));

      // Should have attempted to read config file
  ...
  describe('Error Handling', () => {
    ...
    it('should handle file reading errors gracefully', async () => {
      ...
      expect(readFileSpy).toHaveBeenCalledWith('/nonexistent/config.json', 'utf-8');
