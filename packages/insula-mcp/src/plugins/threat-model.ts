...
export const ThreatModelPlugin: DiagnosticPlugin = {
  ...
  async run(ctx) {
    ...
    const nameSurface = rawTools.map((tool) => (typeof tool?.name === "string" ? tool.name : "")).join(" ").toLowerCase();
    ...
    const networkNames = rawTools
      .map((tool) => (typeof tool?.name === "string" ? tool.name.toLowerCase() : ""))
      .filter((name) => /\b(curl|http|fetch|get|post|upload|download)\b/.test(name));
    ...
    const exfilSoft = !exfilHard && /\b(curl|http|fetch|get|post|upload|download)\b/i.test(descSurface);
    const exfilIsAllowlisted =
      exfilHard && networkNames.every((name) => NETWORK_ALLOWLIST.has(name));

    const privilegeHard = /\b(exec|shell|sudo|chmod|chown|fs|file)\b/i.test(nameSurface);
    const privilegeSoft =
      !privilegeHard && /\b(exec|shell|sudo|chmod|chown|fs|file)\b/i.test(descSurface);

    const mutationRisk = /mutable|dynamic/i.test(descSurface);

    type Level = "info" | "minor" | "major";
    const checks: Array<{ id: string; level: Level; text: string; riskDescription?: string }> = [
      { id: "stride.spoofing.auth", level: "info", text: "Auth scheme validated (G1)" },
      {
        id: "stride.tampering.muttools",
        level: mutationRisk ? "minor" : "info",
        text: "Tool mutation guarded",
      },
      { id: "stride.repudiation.logging", level: "info", text: "Structured, redacted logs" },
      {
        id: "stride.info.disclosure",
        level: exfilHard ? (exfilIsAllowlisted ? "minor" : "major") : exfilSoft ? "minor" : "info",
        text: "Network exfiltration surface",
        riskDescription: "Ensure network-capable tools require explicit scopes and confirmations.",
      },
      { id: "stride.dos.streaming", level: "info", text: "Backpressure/heartbeat in streaming" },
      {
        id: "stride.elevation",
        level: privilegeHard ? "major" : privilegeSoft ? "minor" : "info",
        text: "No exec/fs admin tools without prompts",
        riskDescription: "Exec/file tools demand multi-step confirmation and allowlists.",
      },
    ];
    ...
    for (const check of checks) {
      ...
      findings.push({
        ...
        area: "threat-model",
        severity,
        title: severity === "info" ? `Check: ${check.text}` : `Risk: ${check.text}`,
        description:
          severity === "info"
