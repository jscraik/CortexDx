        ...
        if (ideContext.fileContent) {
            ...
            if (validation.quickFixes.length > 0) {
                findings.push({
                    ...
                    evidence: [{ type: "file", ref: ideContext.filePath || "current-file" }],
                    recommendation:
...
async function validateMcpCode(
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (line.includes("tools:") || (line.includes("export const") && line.includes("Tool"))) {
            // Validate tool structure
            if (!content.includes("inputSchema") && !content.includes("input_schema")) {
                diagnostics.push({
                    range: {
        ...
        if (line.includes("async") && !line.includes("await") && i < lines.length - 10) {
