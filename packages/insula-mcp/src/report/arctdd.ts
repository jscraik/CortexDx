...
export function buildArcTddPlan(stamp: Record<string, unknown>, findings: Finding[]): string {
  ...
  const sections = [
    ...
    blockers.length === 0 ? "✅ No blocking issues found!" : "",
    ...blockers.map((finding) => formatIssueWithSolution(finding, "BLOCKER")),
    "",
    "## ⚠️ Major Issues",
    majors.length === 0 ? "✅ No major issues found!" : "",
    ...majors.map((finding) => formatIssueWithSolution(finding, "MAJOR")),
    "",
    "## 📋 Minor Issues & Improvements",
    minors.length === 0 ? "✅ No minor issues found!" : "",
    ...minors.slice(0, 5).map((finding) => formatIssueWithSolution(finding, "MINOR")),
    minors.length > 5 ? `... and ${minors.length - 5} more minor issues (see full report)` : "",
    "",
...
function formatIssueWithSolution(finding: Finding, severity: string): string {
  ...
  const evidence = finding.evidence?.map(e => `${e.type}:${e.ref}`).join(", ") || "N/A";
...
function generateSolutionSteps(finding: Finding): string[] {
  ...
  if (id.includes("gov.pack.missing")) {
    return [
      "**Create .insula directory:** Add governance configuration in project root",
...
function getQuickWins(findings: Finding[]): string[] {
  ...
  if (findings.some(f => f.id.includes("cors"))) {
    quickWins.push("• **5 min:** Add basic CORS headers to fix cross-origin requests");
  }

  if (findings.some(f => f.id.includes("jsonrpc.no_ping"))) {
    quickWins.push("• **2 min:** Add simple `rpc.ping` method that returns `pong`");
  }

  if (findings.some(f => f.id.includes("batch.req.error"))) {
    quickWins.push("• **10 min:** Fix routing to ensure `/mcp` returns JSON not HTML");
  }

  if (findings.some(f => f.id.includes("disc.unknown"))) {
    quickWins.push("• **15 min:** Implement basic `tools/list` endpoint with empty array");
  }

  if (quickWins.length === 0) {
    quickWins.push("• **Great job!** No obvious quick wins - your MCP server is well implemented");
  }
