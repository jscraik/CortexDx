...
export class OpenAlexProvider {
        ...
        return [
            {
                ...
                inputSchema: {
                    ...
                    properties: {
                        ...
                        filter: {
                            type: "string",
                            description: "Filter criteria (e.g., 'publication_year:2020-2023')"
                        },
                        sort: {
                            type: "string",
                            description: "Sort order (e.g., 'cited_by_count:desc', 'publication_date:desc')",
            ...
            },
            {
                ...
                inputSchema: {
                    ...
                    properties: {
                        ...
                        filter: {
                            type: "string",
                            description: "Filter criteria (e.g., 'last_known_institution.country_code:US')"
                        },
                        sort: {
                            type: "string",
                            description: "Sort order (e.g., 'cited_by_count:desc', 'works_count:desc')",
                            default: "cited_by_count:desc"
                        },
                        page: {
                            type: "number",
            ...
            },
            {
                ...
                inputSchema: {
                    ...
                    properties: {
                        ...
                        filter: {
                            type: "string",
                            description: "Filter criteria (e.g., 'country_code:US', 'type:education')"
                        },
                        sort: {
                            type: "string",
                            description: "Sort order (e.g., 'works_count:desc', 'cited_by_count:desc')",
     ...
     * Search for works
     */
    async searchWorks(params: OpenAlexSearchParams): Promise<{ results: OpenAlexWork[]; meta: unknown }> {
        ...
        if (params.per_page) searchParams.append("per-page", String(params.per_page));
        ...
        try {
            ...
            throw new Error(`OpenAlex API error: ${error instanceof Error ? error.message : "Unknown error"}`);
        }
    }
     ...
     * Search for authors
     */
    async searchAuthors(params: OpenAlexSearchParams): Promise<{ results: OpenAlexAuthor[]; meta: unknown }> {
        const searchParams = new URLSearchParams();
     ...
     */
    async searchInstitutions(params: OpenAlexSearchParams): Promise<{ results: OpenAlexInstitution[]; meta: unknown }> {
        const searchParams = new URLSearchParams();
    ...
    async getWork(id: string): Promise<OpenAlexWork | null> {
        const workId = id.startsWith("https://openalex.org/") ? id : `https://openalex.org/${id}`;
        const url = `${this.baseUrl}/works/${encodeURIComponent(workId)}`;

        try {
            const response = await this.ctx.request<OpenAlexWork>(url, {
                headers: {
                    "User-Agent": this.userAgent,
                    ...this.ctx.headers
                }
    ...
    async getAuthor(id: string): Promise<OpenAlexAuthor | null> {
        const authorId = id.startsWith("https://openalex.org/") ? id : `https://openalex.org/${id}`;
        const url = `${this.baseUrl}/authors/${encodeURIComponent(authorId)}`;

        try {
            const response = await this.ctx.request<OpenAlexAuthor>(url, {
                headers: {
                    "User-Agent": this.userAgent,
                    ...this.ctx.headers
                }
