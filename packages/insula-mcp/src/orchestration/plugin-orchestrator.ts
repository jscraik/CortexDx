...
export class PluginOrchestrator {
     ...
     */
    createWorkflow(definition: Omit<PluginWorkflow, "id">): PluginWorkflow {
    ...
    private getPlugin(
        ...
        if (this.isDevelopmentContext(context)) {
            return getDevelopmentPluginById(pluginId) || getPluginById(pluginId);
        }
    ...
    private evaluateCondition(
        ...
        switch (condition.type) {
            case "severity": {
                ...
                return this.compareValues(count, condition.operator, Number(condition.value) || 0);
            }
            case "finding_count": {
