...
import { WikidataProvider, wikidataCapabilities } from "../../providers/academic/wikidata.mcp.js";
...
export class AcademicRegistry {
    ...
    private registerAllProviders(): void {
        ...
        this.registerProvider({
            ...
            health_check_endpoint: "https://api.semanticscholar.org/graph/v1/paper/search?query=test&limit=1",
            requires_auth: false,
            rate_limits: {
        ...
        this.registerProvider({
            ...
            health_check_endpoint: "https://www.wikidata.org/w/api.php?action=wbsearchentities&search=test&limit=1&format=json",
            requires_auth: false,
            rate_limits: {
                requests_per_minute: 200,
                requests_per_hour: 5000
            },
            tags: ["knowledge-graph", "sparql", "entities", "academic", "researchers"]
        });
        ...
        this.registerProvider({
            ...
            health_check_endpoint: "http://export.arxiv.org/api/query?search_query=cat:cs.AI&max_results=1",
            requires_auth: false,
            rate_limits: {
     ...
     */
    public createProviderInstance(id: string, ctx: DiagnosticContext): ProviderInstance {
     ...
     */
    public async performHealthChecks(ctx: DiagnosticContext): Promise<Record<string, boolean>> {
...
export const academicRegistryCapabilities = {
    ...
    version: "1.0.0",
    description: "Centralized registry for academic research MCP providers with discovery and health check capabilities",
    tools: [
        {
            ...
            inputSchema: {
                ...
                properties: {
                    provider_ids: {
                        type: "array",
                        items: { type: "string" },
                        description: "Specific provider IDs to check (optional, checks all if not specified)"
                    }
                }
            }
        }
