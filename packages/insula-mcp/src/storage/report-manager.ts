...
export class ReportManager {
    ...
    async retrieveReport(reportId: string, format: ReportFormat): Promise<string> {
        if (!this.db) {
            await this.initialize();
        }
    ...
    private getStoragePath(
        ...
        switch (options.organizationStrategy) {
            case "date": {
                ...
                return join(base, "reports", date, `session-${report.sessionId}`, reportId);
            }
            case "session":
                return join(base, "reports", `session-${report.sessionId}`, reportId);
            case "type":
                return join(base, "reports", report.diagnosticType, reportId);
            case "custom":
