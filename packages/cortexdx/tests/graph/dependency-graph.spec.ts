...
import type { ManifestLike, ProbeInventory, DepGraph } from '../src/graph/dependency-graph.js';
  ...
  describe('buildDependencyGraph', () => {
    ...
    it('should handle multiple dependency relations', () => {
      ...
      expect(graph.edges.some(e =>
        e.from === 'tool1' && e.to === 'tool2' && e.relation === 'depends_on'
      )).toBe(true);
      expect(graph.edges.some(e =>
    ...
    it('should not duplicate edges', () => {
      ...
      const probes: ProbeInventory = {
        dependencies: [
          { from: 'toolA', to: 'toolB', relation: 'depends_on' }
        ]
      };
    ...
    it('should create service nodes from resources', () => {
      ...
      expect(graph.edges.some(e =>
        e.from === 'cache' && e.to === 'database' && e.relation === 'depends_on'
      )).toBe(true);
    });
  });
