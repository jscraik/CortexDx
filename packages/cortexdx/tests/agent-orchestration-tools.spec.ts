...
import { createAgentOrchestrationTools, executeAgentOrchestrationTool } from "../src/tools/agent-orchestration-tools.js";
...
const createStateDbPath = (): { dir?: string; dbPath: string } => ({ dbPath: ":memory:" });
...
describe("Agent orchestration MCP tools", () => {
    ...
    it("exposes listing, execution, and history tools", async () => {
        ...
        const listTool = tools.find((tool) => tool.name === "cortexdx_agent_list_workflows")!;
        const listResult = await executeAgentOrchestrationTool(listTool, { stateDb }, context);
    ...
    it("runs workflows and records checkpoint history", async () => {
        const tools = createAgentOrchestrationTools();
        const executeTool = tools.find((tool) => tool.name === "cortexdx_agent_execute_workflow")!;
        ...
        const historyTool = tools.find((tool) => tool.name === "cortexdx_agent_checkpoint_history")!;
