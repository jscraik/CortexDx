...
import type { ResolutionPattern, FeedbackEntry, CommonIssuePattern } from '../../src/storage/pattern-storage.js';

// Test encryption key (32 bytes hex)
const TEST_ENCRYPTION_KEY = 'a'.repeat(64); // 64 hex chars = 32 bytes

// Mock resolution pattern
const createMockPattern = (overrides?: Partial<ResolutionPattern>): ResolutionPattern => ({
  id: 'pattern-123',
...
describe('Storage - Pattern Encryption', () => {
  ...
  describe('PatternAnonymizer', () => {
    it('should anonymize URLs', () => {
      const signature = 'Failed to connect to https://api.internal.company.com/v1/endpoint';
    ...
    it('should anonymize bearer tokens', () => {
      const signature = 'Authentication failed: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9';
      const anonymized = PatternAnonymizer.anonymizeProblemSignature(signature);
    ...
    it('should anonymize solution arrays', () => {
      ...
      const anonymized = PatternAnonymizer.anonymizeSolution(solution) as string[];
  ...
  describe('Feedback Management', () => {
    ...
    it('should update pattern confidence based on feedback', async () => {
      const pattern = createMockPattern({ successCount: 10, failureCount: 2, confidence: 0.83 });
      await storage.savePattern(pattern);
  ...
  describe('Common Issues Tracking', () => {
    ...
    it('should anonymize issue signatures', async () => {
      const issue: CommonIssuePattern = {
        signature: 'Failed to auth with https://api.company.com using token abc123',
        occurrences: 1,
        solutions: [],
        contexts: [],
        firstSeen: Date.now(),
  ...
  describe('Pattern Retrieval and Filtering', () => {
    beforeEach(async () => {
      ...
      await storage.savePattern(createMockPattern({ id: 'high-conf', confidence: 0.9, successCount: 10 }));
      await storage.savePattern(createMockPattern({ id: 'low-conf', confidence: 0.3, successCount: 2 }));
      await storage.savePattern(createMockPattern({ id: 'old-pattern', lastUsed: Date.now() - 90 * 24 * 60 * 60 * 1000 }));
    });

    it('should retrieve patterns by minimum confidence', async () => {
      const patterns = await storage.retrievePatternsByRank({ minConfidence: 0.7 });
    ...
    it('should retrieve patterns by minimum success count', async () => {
      const patterns = await storage.retrievePatternsByRank({ minSuccessCount: 5 });
    ...
    it('should filter patterns by age', async () => {
      ...
      expect(patterns.every((p) => p.lastUsed >= Date.now() - maxAge)).toBe(true);
    });

    it('should sort patterns by confidence', async () => {
      const patterns = await storage.retrievePatternsByRank({ sortBy: 'confidence' });
  ...
  describe('Pattern Statistics', () => {
    it('should calculate pattern statistics', async () => {
      await storage.savePattern(createMockPattern({ id: 'p1', successCount: 10, failureCount: 2 }));
      await storage.savePattern(createMockPattern({ id: 'p2', successCount: 5, failureCount: 1 }));
    ...
    it('should identify most successful pattern', async () => {
      await storage.savePattern(createMockPattern({ id: 'best', successCount: 20 }));
      await storage.savePattern(createMockPattern({ id: 'good', successCount: 10 }));
    ...
    it('should track recently used patterns', async () => {
      const now = Date.now();
      await storage.savePattern(createMockPattern({ id: 'recent', lastUsed: now }));
      await storage.savePattern(createMockPattern({ id: 'old', lastUsed: now - 100000 }));
    ...
    it('should group patterns by type', async () => {
      await storage.savePattern(createMockPattern({ id: 'p1', problemType: 'connection' }));
      await storage.savePattern(createMockPattern({ id: 'p2', problemType: 'connection' }));
      await storage.savePattern(createMockPattern({ id: 'p3', problemType: 'auth' }));
  ...
  describe('Similar Pattern Matching', () => {
    beforeEach(async () => {
      await storage.savePattern(createMockPattern({
        id: 'conn-1',
    ...
    it('should find similar patterns', async () => {
      const similar = await storage.findSimilarPatterns('Failed to connect to MCP server');
    ...
    it('should respect similarity threshold', async () => {
      const highThreshold = await storage.findSimilarPatterns('MCP connection issue', 0.9);
      const lowThreshold = await storage.findSimilarPatterns('MCP connection issue', 0.1);

      expect(lowThreshold.length).toBeGreaterThanOrEqual(highThreshold.length);
    });

    it('should not match dissimilar patterns', async () => {
      const similar = await storage.findSimilarPatterns('Database query timeout', 0.6);
  ...
  describe('Pattern Pruning', () => {
    it('should prune old patterns', async () => {
      ...
      await storage.savePattern(createMockPattern({ id: 'old', lastUsed: oneYearAgo }));
      await storage.savePattern(createMockPattern({ id: 'recent', lastUsed: now }));
    ...
    it('should not prune recent patterns', async () => {
      ...
      await storage.savePattern(createMockPattern({ id: 'recent-1', lastUsed: now - 1000 }));
      await storage.savePattern(createMockPattern({ id: 'recent-2', lastUsed: now - 2000 }));
