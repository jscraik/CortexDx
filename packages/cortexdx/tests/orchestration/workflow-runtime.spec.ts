...
import { AgentOrchestrator, type WorkflowDefinition } from '../../src/orchestration/agent-orchestrator.js';
import { StateManager, type StatePersistenceConfig } from '../../src/orchestration/state-manager.js';
...
describe('Orchestration - Workflow Runtime', () => {
  ...
  describe('Workflow Creation', () => {
    ...
    it('should enable checkpointing when configured', () => {
      const definition: WorkflowDefinition = {
        ...
        nodes: [
          { id: 'node1', name: 'Node 1', type: 'aggregation' },
        ],
        edges: [],
        entryPoint: 'node1',
      };
    ...
    it('should register workflow in orchestrator', () => {
      const definition: WorkflowDefinition = {
        ...
        nodes: [
          { id: 'start', name: 'Start', type: 'aggregation' },
        ],
        edges: [],
        entryPoint: 'start',
      };
  ...
  describe('Conditional Branching', () => {
    it('should route based on conditions', async () => {
      ...
      // Test blocker path
      const blockerResult = await orchestrator.executeWorkflow('conditional-workflow', {
      ...
      // Test normal path
      const normalResult = await orchestrator.executeWorkflow('conditional-workflow', {
  ...
  describe('State Management', () => {
    it('should persist state checkpoints', async () => {
      ...
      const checkpoint = await stateManager.loadCheckpoint(result.checkpointId!);
    ...
    it('should recover state from checkpoint', async () => {
      ...
      const result = await orchestrator.executeWorkflow('recovery-test', {
        context,
        endpoint: context.endpoint,
  ...
  describe('Workflow Management', () => {
    it('should list all workflows', () => {
      const definitions: WorkflowDefinition[] = [
        {
          config: { workflowId: 'wf1', name: 'Workflow 1', description: 'Test 1', enableCheckpointing: false },
          nodes: [{ id: 'n1', name: 'Node 1', type: 'aggregation' }],
          edges: [],
          entryPoint: 'n1',
        },
        {
          config: { workflowId: 'wf2', name: 'Workflow 2', description: 'Test 2', enableCheckpointing: false },
