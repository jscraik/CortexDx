...
describe('compliance-scoring', () => {
  describe('SCORING_CONFIG', () => {
    it('should have valid penalty structure', () => {
      expect(SCORING_PENALTIES.blocker).toBeGreaterThanOrEqual(SCORING_PENALTIES.major);
      expect(SCORING_PENALTIES.major).toBeGreaterThanOrEqual(SCORING_PENALTIES.minor);
      expect(SCORING_PENALTIES.minor).toBeGreaterThanOrEqual(SCORING_PENALTIES.info);
    });
  ...
  describe('calculateComplianceScore', () => {
    it('should return perfect score for no findings', () => {
      ...
      expect(result.counts).toEqual({ blocker: 0, major: 0, minor: 0, info: 0 });
    });
    ...
    it('should assign correct grades', () => {
      ...
      expect(calculateComplianceScore([{ severity: 'minor', area: 'X', title: 'Y' }]).grade).toBe('A');
      // Single major finding: score = 90, which is still A (90+ threshold)
      expect(calculateComplianceScore([{ severity: 'major', area: 'X', title: 'Y' }]).grade).toBe('A');
      ...
      expect(calculateComplianceScore([{ severity: 'blocker', area: 'X', title: 'Y' }]).grade).toBe('B');
  ...
  describe('reproducibility', () => {
    ...
    it('should produce deterministic results across multiple runs', () => {
      ...
      const scores = Array(10).fill(0).map(() => calculateComplianceScore(findings));
