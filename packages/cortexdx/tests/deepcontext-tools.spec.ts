...
} from "../src/tools/deepcontext-tools.js";
...
describe("DeepContext MCP tools", () => {
  ...
  it("indexes via MCP tool", async () => {
    const tool = createDeepContextTools().find(
      (t) => t.name === "cortexdx_deepcontext_index",
    );
    expect(tool).toBeDefined();
    const result = await executeDeepContextTool(
      tool!,
      { codebasePath: "/repo", force: true },
      baseCtx,
    );
  ...
  it("searches via MCP tool", async () => {
    ...
    const tool = createDeepContextTools().find(
      (t) => t.name === "cortexdx_deepcontext_search",
    );
    expect(tool).toBeDefined();
    const result = await executeDeepContextTool(
      tool!,
  ...
  it("reports status via MCP tool", async () => {
    const tool = createDeepContextTools().find(
      (t) => t.name === "cortexdx_deepcontext_status",
    );
    expect(tool).toBeDefined();
  ...
  it("clears via MCP tool", async () => {
    const tool = createDeepContextTools().find(
      (t) => t.name === "cortexdx_deepcontext_clear",
    );
    expect(tool).toBeDefined();
