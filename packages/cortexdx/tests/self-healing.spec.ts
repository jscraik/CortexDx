...
} from "../src/templates/fix-templates.js";
...
describeIntegration("Self-Healing Suite", () => {
  describe("AutoHealer", () => {
    ...
    describe("quickHealthCheck", () => {
      ...
      it("should handle diagnostic errors gracefully", async () => {
        vi.spyOn(healer as any, "runDiagnostics").mockRejectedValue(
    ...
    describe("getHealingRecommendations", () => {
      it("should provide recommendations for findings", async () => {
        ...
        vi.spyOn(healer as any, "analyzeFindings").mockResolvedValue(
          mockFindings,
        );
        ...
        expect(result.recommendations).toContain(
          "Apply Add Security Headers (5-10 minutes)",
        );
    ...
    describe("healSelf", () => {
      it("should run complete healing cycle successfully", async () => {
        ...
        vi.spyOn(healer as any, "applyAutomatedFixes").mockResolvedValue(
          mockReport.fixes,
        );
        vi.spyOn(healer as any, "validateFixes").mockResolvedValue(
          mockReport.validation,
        );
        vi.spyOn(healer as any, "generateSummary").mockResolvedValue(
          mockReport.summary,
        );
  ...
  describe("InspectorAdapter", () => {
    ...
    describe("diagnose", () => {
      it("should run diagnostics and return report", async () => {
        const result = await adapter.diagnose("http://localhost:3000", [
          "handshake",
          "security",
        ]);
      ...
      it("should handle endpoint errors gracefully", async () => {
        const result = await adapter.diagnose("invalid-endpoint", [
          "handshake",
        ]);
  ...
  describe("Error Handling", () => {
    ...
    it("should handle auto-healer errors gracefully", async () => {
      ...
      vi.spyOn(healer as any, "runDiagnostics").mockRejectedValue(
        new Error("Diagnostic service unavailable"),
      );
