...
import { WorkflowVisualizationEngine } from "../src/orchestration/workflow-visualization.js";
...
describe("LangGraph Integration", () => {
  ...
  describe("Human-in-the-Loop (Req 18.4)", () => {
    it("should pause workflow at decision point", async () => {
      ...
      expect(humanInLoop.isWorkflowPaused("test-workflow", "thread-1")).toBe(
        true,
      );
    });
  ...
  describe("Workflow Visualization (Req 18.5)", () => {
    it("should generate Mermaid diagram from workflow", () => {
      const definition: WorkflowDefinition = {
        ...
        edges: [{ from: "node1", to: "node2", label: "next" }],
        entryPoint: "node1",
      };
