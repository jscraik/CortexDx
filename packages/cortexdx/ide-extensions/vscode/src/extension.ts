...
export function activate(context: vscode.ExtensionContext): void {
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand("cortexdx.analyzeProject", analyzeProject),
    vscode.commands.registerCommand("cortexdx.generateTests", generateTests),
...
export function deactivate(): vscode.Thenable<void> | undefined {
  if (!client) {
    return undefined;
  }
  return client.stop();
...
async function startLanguageClient(
  ...
  const clientOptions: LanguageClientOptions = {
    ...
    synchronize: {
      fileEvents:
        vscode.workspace.createFileSystemWatcher("**/*.{ts,js,py,go}"),
    },
  };
...
async function connectToServer(_url: string, _transport: string): Promise<any> {
