...
export const ValidationRules: Record<string, ValidationRule> = {
  'security.headers.present': {
    ...
    validator: async (context) => {
      ...
      const hasHeaders = context.securityHeaders?.length > 0;
  ...
  'sse.streaming.healthy': {
    ...
    validator: async (context) => {
      ...
      return {
        passed: sseHealthy && heartbeatWorking,
        message: (sseHealthy && heartbeatWorking)
          ? 'SSE streaming is healthy with heartbeat'
          : 'SSE streaming has issues',
        details: { sseHealthy, heartbeatWorking },
        suggestions: (!sseHealthy || !heartbeatWorking) ? [
  ...
  'jsonrpc.compliant': {
    ...
    validator: async (context) => {
      ...
      const hasRequiredFields = requiredFields.every(field => compliance[field]);
  ...
  'conversation.storage.working': {
    ...
    validator: async (context) => {
      ...
      return {
        passed: storageWorking && persistenceVerified,
        message: (storageWorking && persistenceVerified)
        ...
        suggestions: (!storageWorking || !persistenceVerified) ? [
  ...
  'functionality.preserved': {
    ...
    validator: async (context) => {
      ...
      const allWorking = coreFeatures.length > 0 && coreFeatures.every((feature: string) =>
  ...
  'tests.passing': {
    ...
    validator: async (context) => {
      ...
      const testResults: TestResult[] = Array.isArray(rawResults) ? rawResults : [];
  ...
  'performance.acceptable': {
    ...
    validator: async (context) => {
      ...
      const responseTimeThreshold = context.performance?.responseTimeThreshold || 1000; // 1s
      const throughputThreshold = context.performance?.throughputThreshold || 10; // 10 req/s
      ...
      return {
        ...
        message: (responseTimeOk && throughputOk)
          ? 'Performance metrics are acceptable'
          : 'Performance metrics need attention',
        details: { responseTime, throughput, responseTimeThreshold, throughputThreshold },
        suggestions: (!responseTimeOk || !throughputOk) ? [
...
export function getValidationRulesBySeverity(severity: ValidationRule['severity']): ValidationRule[] {
  return Object.values(ValidationRules).filter(rule => rule.severity === severity);
}
