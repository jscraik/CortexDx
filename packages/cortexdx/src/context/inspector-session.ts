...
export function createInspectorSession(
  ...
  const applySessionId = (value?: string | null) => {
    ...
    if (shared) shared.sessionId = normalized;
  };
  ...
  const ensureSession = async () => {
    ...
    if (!transcript.sessionId) applySessionId(randomUUID());
  ...
  const probeStream = async (
    ...
    for (const candidate of candidates) {
      ...
      const withQuery = `${candidate}${separator}mcp-session-id=${encodeURIComponent(
        sessionId,
      )}`;
