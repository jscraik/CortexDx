...
export class ReportServer {
  ...
  private cache: Map<
    string,
    { content: string; compressed: Buffer; timestamp: number }
  > = new Map();
  ...
  async start(): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      server.listen(this.options.port, this.options.host, () => {
        console.log(
          `Report server listening on http://${this.options.host}:${this.options.port}`,
        );
        resolve();
      });
  ...
  private async handleRequest(
    ...
    try {
      ...
      const format = this.negotiateFormat(
        req,
        url.query.format as string | undefined,
      );
  ...
  private async sendResponse(
    ...
    if (this.options.enableCaching) {
      res.setHeader(
        "Cache-Control",
        `public, max-age=${this.options.cacheMaxAge}`,
      );
  ...
  private sendCachedResponse(
    ...
    if (this.options.enableCompression && acceptEncoding.includes("gzip")) {
      ...
      res.setHeader(
        "Content-Length",
        Buffer.byteLength(cached.content, "utf-8"),
      );
      res.writeHead(200);
