...
class CortexDxClient {
  ...
  async runDiagnostics() {
    ...
    const suites = Array.from(suitesSelect.selectedOptions).map(
      (opt) => opt.value,
    );
    ...
    resultsPanel.innerHTML =
      '<div class="loading"></div> Running diagnostics...';
  ...
  displayDiagnosticResults(result) {
    ...
    try {
      ...
      if (!findings || !findings.findings || findings.findings.length === 0) {
        resultsPanel.innerHTML =
          '<div class="finding info"><div class="finding-title">No Issues Found</div><div class="finding-description">The MCP server appears to be functioning correctly.</div></div>';
        return;
      }

      const html = findings.findings
        .map(
          (finding) => `
  ...
  displayPlugins(providers) {
    ...
    const html = Object.entries(providers)
      .map(
        ([id, provider]) => `
