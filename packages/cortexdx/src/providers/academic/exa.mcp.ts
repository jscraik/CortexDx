...
export class ExaProvider {
  ...
  static getToolDefinitions() {
    return [
      ...
      },
      {
        ...
        description:
          "Validate search result relevance using Finding confidence system",
        inputSchema: {
          type: "object",
          properties: {
            results: {
  ...
  async search(params: ExaSearchParams): Promise<ExaSearchResult[]> {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Exa API error ${response.status}: ${errorText.slice(0, 200)}`,
      );
    }
  ...
  private buildSearchPayload(params: ExaSearchParams): Record<string, unknown> {
    ...
    if (filters?.dateRange?.start)
      payload.startPublishedDate = filters.dateRange.start;
    if (filters?.dateRange?.end)
      payload.endPublishedDate = filters.dateRange.end;
    if (filters?.sources?.length) payload.includeDomains = filters.sources;
    if (filters?.contentType?.length)
      payload.documentTypes = filters.contentType;
    return payload;
  }
    ...
    switch (toolName) {
      case "exa_search":
        return await this.search({
          ...
          filters: params.filters as ExaSearchParams["filters"] | undefined,
          includeAnalysis: params.includeAnalysis as boolean | undefined,
        });
      ...
      case "exa_validate_relevance":
        return await this.validateRelevance(
          params.results as ExaSearchResult[],
          params.query as string,
          params.minConfidence as number | undefined,
        );
  ...
  prompts: [
    {
      ...
      description:
        "Perform advanced search with quality and relevance analysis",
      arguments: [
        {
          name: "query",
          description: "Search query",
