...
async function main() {
  ...
  const outputDir = path.join(process.cwd(), "reports", "self-healing", timestamp);
  ...
  try {
    ...
    console.error("Self-healing suite failed:", error instanceof Error ? error.message : error);
    process.exit(3);
  }
}
