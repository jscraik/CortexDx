...
export function scoreConfidence(event: Event, context: ConfidenceContext = {}): number {
  const base = KIND_BASE[event.kind] ?? 0.5;
  const magnitude = clamp(Number(event.meta.magnitude ?? event.meta.delta ?? 0.2));
...
function countEvidence(evidence: ConfidenceContext["evidence"]): number {
  ...
  return (evidence.logs?.length ?? 0) + (evidence.traces?.length ?? 0) + (evidence.metrics?.length ?? 0);
}
