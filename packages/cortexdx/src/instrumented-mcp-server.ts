...
export async function createInstrumentedMcpServer() {
  const telemetryToken = process.env.SHINZO_TELEMETRY_TOKEN;

  if (!telemetryToken) {
    throw new Error(
      "SHINZO_TELEMETRY_TOKEN is required to start the instrumented MCP server",
    );
  }
    ...
    "cortexdx_diagnose",
    {
      ...
      inputSchema: {
        ...
        suites: z
          .array(z.string())
          .optional()
          .describe("Diagnostic suites to run"),
      },
      outputSchema: {
        status: z.string(),
        endpoint: z.string(),
...
if (
  ...
  process.argv[1] &&
  process.argv[1].endsWith("instrumented-mcp-server.ts")
) {
