            ...
            for (const [signature, problemGroup] of signatureMap.entries()) {
                if (problemGroup.length >= 2) {
                    const solutions = problemGroup
                        .flatMap((p) => p.suggestedSolutions.map((s) => s.solution.description))
                    ...
                    commonPatterns.push({
                        ...
                        solutions,
                        contexts,
                        firstSeen: Math.min(...problemGroup.map((p) => p.context.sessionContext?.startTime || Date.now())),
                        lastSeen: Math.max(...problemGroup.map((p) => p.context.sessionContext?.lastActivity || Date.now())),
                    });
                }
            }
    ...
    return {
        async learnFromSuccess(
            ...
            if (existingPattern) {
                ...
                existingPattern.confidence = existingPattern.successCount / totalAttempts;
            } else {
                // Create new pattern
                patterns.push({
