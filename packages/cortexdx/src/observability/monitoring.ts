 ...
 */
function metricsToRecord<T>(metrics: T): Record<string, unknown> {
...
export class MonitoringSystem {
  ...
  private checkAlerts(health: SystemHealthReport): void {
    ...
    // Check memory usage
    if (
      health.metrics.memory.heapUsagePercent >
    ...
    if (
      ...
      const severity =
        health.metrics.performance.avgResponseTimeMs > 5000
          ? "critical"
          : "warning";
    ...
    // Check error rate
    if (
      health.metrics.performance.errorRate >
      this.config.alertThresholds.errorRatePercent
    ) {
      const severity =
        health.metrics.performance.errorRate > 10 ? "critical" : "warning";
