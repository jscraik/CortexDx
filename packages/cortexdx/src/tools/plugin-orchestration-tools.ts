...
import {
  type PluginWorkflow,
  type WorkflowStage,
  getPluginOrchestrator,
} from "../orchestration/plugin-orchestrator.js";
import type {
  DevelopmentContext,
  DiagnosticContext,
  Finding,
...
export function createPluginOrchestrationTools(): McpTool[] {
  ...
  return [
    ...
    // Create workflow
    {
      ...
      description:
        "Create a new diagnostic workflow with stages and dependencies",
      inputSchema: {
        type: "object",
        properties: {
          name: {
...
async function executePlugin(
  ...
  try {
    ...
    const findings = await orchestrator.executePlugin(
      pluginId,
      executionContext,
    );
    ...
    const { pluginId, endpoint } = args as {
      pluginId?: string;
      endpoint?: string;
    };
    const message = error instanceof Error ? error.message : String(error);
    throw new Error(
      `Failed to execute plugin ${pluginId || "unknown"} on ${endpoint || "unknown endpoint"}: ${message}`,
    );
  }
}
...
async function executeParallel(
  ...
  try {
    ...
    const { pluginIds, endpoint } = args as {
      pluginIds?: string[];
      endpoint?: string;
    };
    const message = error instanceof Error ? error.message : String(error);
    throw new Error(
      `Failed to execute plugins in parallel (${pluginIds?.join(", ") || "unknown"}) on ${endpoint || "unknown endpoint"}: ${message}`,
    );
  }
}
...
async function executeWorkflow(
  ...
  try {
    ...
    const { workflowId, endpoint } = args as {
      workflowId?: string;
      endpoint?: string;
    };
    const message = error instanceof Error ? error.message : String(error);
    throw new Error(
      `Failed to execute workflow ${workflowId || "unknown"} on ${endpoint || "unknown endpoint"}: ${message}`,
    );
  }
}
