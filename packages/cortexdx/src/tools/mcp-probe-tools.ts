    ...
    inputSchema: {
      ...
      properties: {
        targetUrl: {
          type: "string",
          description:
            "URL of the MCP server to probe (e.g., https://other-mcp-server.com)",
        },
        auth: {
          type: "object",
          description:
            "Auth0 configuration for authenticating with the target server",
          properties: {
            domain: {
              type: "string",
            ...
            audience: {
              type: "string",
              description:
                "Auth0 audience (e.g., https://target-server.com/api)",
            },
            scope: {
              type: "string",
        ...
        suites: {
          ...
          items: { type: "string" },
          description:
            "Diagnostic suites to run (e.g., ['protocol', 'security', 'performance']). Runs all if not specified.",
        },
        full: {
          type: "boolean",
          description:
            "Run full comprehensive diagnostics (all plugins, default: false)",
        },
        timeout: {
          type: "number",
...
function groupFindingsByArea(
  findings: Array<{ area: string; severity: string }>,
