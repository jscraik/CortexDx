  ...
  return [
    {
      ...
      description:
        "List available LangGraph agent workflows and checkpointing metadata",
      inputSchema: {
        type: "object",
        properties: {
          stateDb: {
    ...
    },
    {
      ...
      description:
        "Execute a LangGraph agent workflow with optional resume + streaming",
      inputSchema: {
        type: "object",
        properties: {
...
async function listAgentWorkflows(params: {
  stateDb?: string;
}): Promise<McpToolResult> {
...
async function runAgentWorkflowFromTool(
  ...
  const threadId =
    args.threadId ?? args.resumeThread ?? `agent-thread-${Date.now()}`;
  ...
  const result = await services.agentOrchestrator.executeWorkflow(
    args.workflowId,
    initialState,
    {
      threadId,
