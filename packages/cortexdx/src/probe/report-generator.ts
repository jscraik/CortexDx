...
function generateMarkdownReport(
  ...
  if (score >= 90) {
    md +=
      "🎉 **Excellent!** This MCP server demonstrates high protocol compliance and best practices.\n\n";
  } else if (score >= 70) {
    md +=
      "⚠️ **Good** with room for improvement. Address the findings below to achieve full compliance.\n\n";
  } else if (score >= 50) {
    md +=
      "⚠️ **Needs Attention.** Several compliance and security issues need to be resolved.\n\n";
  } else {
    md +=
      "🚨 **Critical Issues Detected.** Immediate action required to meet MCP protocol standards.\n\n";
  }
  ...
  md +=
    "*For questions or support, visit https://github.com/jscraik/CortexDx*\n";
...
function generateRecommendations(
  ...
  if (summary.criticalFindings > 0) {
    recommendations.push(
      "**Immediate Action Required:** Address all critical (blocker) issues before deploying to production.",
    );
  }

  const protocolIssues = findings.filter(
    (f) => f.area === "protocol" && f.severity !== "info",
  );
  if (protocolIssues.length > 0) {
    recommendations.push(
      "Review and fix protocol compliance issues to ensure compatibility with MCP clients.",
    );
  }
  ...
  if (securityIssues.length > 0) {
    recommendations.push(
      "Address security vulnerabilities to prevent unauthorized access and data breaches.",
    );
  }

  const performanceIssues = findings.filter(
    (f) => f.area === "performance" && f.severity !== "info",
  );
  if (performanceIssues.length > 0) {
    recommendations.push(
      "Optimize performance to meet response time and throughput requirements.",
    );
  }

  if (summary.score < 70) {
    recommendations.push(
      "Consider a comprehensive code review and refactoring to improve overall quality.",
    );
  }

  if (recommendations.length === 0) {
    recommendations.push(
      "Continue monitoring and maintaining best practices. Great work!",
    );
  }
