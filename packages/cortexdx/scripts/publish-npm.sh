#!/usr/bin/env bash
# Publish CortexDx to npm registry
...
echo "📦 Publishing CortexDx v${VERSION} to npm"
echo "Tag: ${NPM_TAG}"
...
if [ ! -f "package.json" ]; then
  echo "❌ Error: package.json not found. Run from packages/cortexdx directory."
  exit 1
fi
...
if [ "${DRY_RUN}" = "true" ]; then
  ...
  echo ""
  echo "✅ Published @brainwav/cortexdx@${VERSION}"
  echo ""
  echo "📦 Package available at:"
  echo "   https://www.npmjs.com/package/@brainwav/cortexdx"
  echo ""
  echo "Install with:"
  echo "   npm install -g @brainwav/cortexdx@${VERSION}"
fi
