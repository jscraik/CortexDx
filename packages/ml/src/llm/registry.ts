...
const BUILTIN_PLUGINS: LLMProviderPlugin[] = [
  ollamaPlugin,
  anthropicPlugin,
  zaiPlugin,
  openaiPlugin,
...
function loadExternalPluginsFromEnv(logger: SecureLogger): LLMProviderPlugin[] {
...
export class LLMPluginRegistry {
  ...
  private createContext(): LLMPluginContext {
    const context: LLMPluginContext = { env: this.env, logger: this.logger };
    if (this.defaults) {
